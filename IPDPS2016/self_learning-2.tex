...
\IEEEauthorblockA{LIG, University Grenoble Alpes\\
Grenoble, France\\
...
assume that the configurable algorithmic parameters are well identified and that there exists benchmarks for their calibration.
...
systems and frameworks~\cite{Rice,Tapus:2002:AHT:762761.762771,ansel:cases:2012}. Despite these contributions, our work brings 
...
such as minimizing the runtime or energy consumption induced by its processing.
...
these approaches are not suitable to NP-complete problems.

Our first disagreement comes from the fact that such an exploration-exploitation tradeoff {\it requires a reference or representative
...
algorithms are estimated on a reference benchmark. On NP-hard problems, this stage could take several days,
months or years.


In the light of these observations, we argue that the first difference between our work and prior ones is to think
automatic tuning in an alternative exploration-exploitation tradeoff, more suitable for NP-hard problems.
...
such that the tuned system evolves throughout the time with new optimal or sub-optimal configurations, discovered in the processing.
...
developed for specific problems.  In the class of problem specific approaches, 
some convincing examples are the ATLAS~\cite{ATLAS} library for matrix
...
FFTW~\cite{FFTW}  for Fast Fourier transform. These works demonstrated the
...
be used in a wide range of NP-complete algorithms.
...
%This solution is particularly interesting since it also addresses the tuning
...
regret function.
%In addition to the contribution of ParamILS, we introduce a
...
tuning as a long-term process. While these works invest on the idea of
...
learning~\cite{DBLP:series/synthesis/2012Settles} literature.  Previous
...
Contrary to QTuning, SATZilla is problem specific, as it requires
...
learning system intends to determine the ratings (here, \textit{runtime}) users
...
is the old Algorithm selection framework of Rice~\cite{Rice}. This work
...
In the next section, we will introduce the key modeling concepts that are
...
configuration, close to the best one. Finally, as other active learning systems~\cite{DBLP:series/synthesis/2012Settles},
...
{\it service.d} request a new configuration to {\it QTunind.d} every $d$ seconds.
